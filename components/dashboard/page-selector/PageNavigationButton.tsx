...
const NavigationButtonSpan = styled.span`
  ...
  border: 0;
  text-align: center;
  background: ${(props: NavigationButtonStyleProps) =>
    props.disabled ? '#c6c6c6' : '#5FCEC0'};
  color: #fff;
  box-shadow: 0 0 0 2px
    ${(props: NavigationButtonStyleProps) => (props.disabled ? '#c6c6c6' : '#5fcec0')};
  width: 10em;
  padding: 0;

  ${NavigationButton}: hover & {
    background: ${(props: NavigationButtonStyleProps) =>
      props.disabled ? '#c6c6c6' : '#3bbfaf'};
  }
`;
...
const PageNavigationButton: FunctionComponent<
  PageNavigationButtonProps & ConnectedProps<typeof connector>
> = ({ updatePage, buttonText, disabled, navigationDirection, page }) => {
  const onClick = () => {
    if (disabled) return;
