...
const Article = ({ frontMatter, children, previousPost, nextPost }) => {
  ...
  return (
    <div className="pt-4 md:pt-10 w-full min-w-full">
      {/* Return button always stays in flow, above the article content, but becomes sticky when scrolling */}
      <div className="mb-6 md:mb-10 max-w-[65ch] md:max-w-[75ch] lg:max-w-[85ch] mx-auto flex items-center sticky top-6 md:top-10 z-10">
        <button
          onClick={handleBackClick}
          className="group flex items-center gap-2 text-secondary hover:text-primary transition-all duration-300 bg-background/80 dark:bg-primary/80 backdrop-blur-sm py-2 pr-4 rounded-full -ml-2 pl-2"
          aria-label="Go back to previous page"
        >
          <div className="relative flex items-center justify-center w-6 h-6 rounded-full border border-accent-subtle/50 group-hover:border-primary/50 group-hover:bg-primary/5 transition-all duration-300 overflow-hidden bg-background dark:bg-primary">
